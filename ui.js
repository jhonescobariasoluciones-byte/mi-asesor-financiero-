...
const formatMoneda = (valor) => valor.toLocaleString('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0 });

const render = () => {
  const totalIngresos = state.ingresos.reduce((total, item) => total + item.valor, 0);
  const totalEgresos = state.egresos.reduce((total, item) => total + item.valor, 0);
  ...
  porcentajeEgresosEl.textContent = state.porcentajeEgresos > 0 ? `${state.porcentajeEgresos}%` : '---';
...
const onFormSubmit = (event) => {
  ...
  if (descripcion.trim() === '' || valor.trim() === '' || isNaN(valor)) {
    alert('Por favor, introduce una descripción y un valor válidos.');
    return;
  }
